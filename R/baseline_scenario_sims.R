BuildMu <- function(species, tsf_values, Time) {
...
BuildPriorityList <- function(priorities) {
...
BuildMatrix <- function(row_values, tsf_values, species) {
...
BuildArrays <- function(species) {
  ...
  priority_list <- BuildPriorityList(priorities)
...
BuildScenario <- function(days, field_day_seq) {
...
SimulateFieldTrips <- function(num_scen) {
  ...
  all_findsthrutime <- array(NA, c(40000, 6, settings$num_reps, num_TSF, num_scen, num_fieldtrips))
  colnames(all_findsthrutime) <- c('ChosenSpecies','num_plants',  'TSF',  'Rep', 'time_mins','H')
  all_summary_data <- array(num_, c(num_species, 6, settings$num_reps, num_TSF, num_scen, num_fieldtrips))
